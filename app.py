...
def generate_data(iterations):
    ...
    for i in range(iterations):
        frequency_samples = np.random.lognormal(mean=np.log(df['Média de Frequência']), sigma=df['Desvio Padrão de Frequência'])
...
def main():
    ...
    iterations = st.sidebar.number_input('Número de Simulações Monte Carlo', min_value=100, max_value=10000, value=1000, step=100)
    ...
    st.dataframe(st.session_state.df[['Evento de Ameaça', 'Risco Médio', 'Percentil 95 do Risco', 'Value at Risk', 'Erro']])
    
    event_choice = st.selectbox('Selecione um Evento de Ameaça para Visualizar Gráficos:', st.session_state.df['Evento de Ameaça'])
